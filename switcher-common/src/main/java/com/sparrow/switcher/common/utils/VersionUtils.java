...
public class VersionUtils {
    ...
    private static final String SPARROW_VERSION_FILE = "sparrow-version.txt";
    
    static {
        try (InputStream in = VersionUtils.class.getClassLoader().getResourceAsStream(SPARROW_VERSION_FILE)) {
            Properties props = new Properties();
            props.load(in);
            String val = props.getProperty("version");
            if (val != null && !VERSION_PLACEHOLDER.equals(val)) {
                version = val;
                clientVersion = "Sparrow-Java-Client:v" + VersionUtils.version;
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
